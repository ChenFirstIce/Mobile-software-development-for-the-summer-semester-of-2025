...
Page({
  ...
  saveAlbumToStorage: function (albumData) {
    ...
    // 如果有封面图片，先上传到云存储
    if (albumData.coverImage && albumData.coverImage.startsWith('file://')) {
      this.uploadCoverImage(albumData)
    } else {
      this.saveAlbumToCloud(albumData)
    }
  },

  // 上传封面图片到云存储
  uploadCoverImage: function (albumData) {
    const fileName = `covers/${Date.now()}_${Math.random().toString(36).substr(2, 9)}.jpg`
    
    wx.cloud.uploadFile({
      cloudPath: fileName,
      filePath: albumData.coverImage,
      success: (res) => {
        console.log('封面图片上传成功:', res)
        // 更新相册数据中的封面图片URL
        albumData.coverImage = res.fileID
        this.saveAlbumToCloud(albumData)
      },
      fail: (error) => {
        console.error('封面图片上传失败:', error)
        app.hideLoading()
        app.showToast('封面图片上传失败')
      }
    })
  },

  // 保存相册到云存储
  saveAlbumToCloud: function (albumData) {
