...
Page({
  ...
  navigateToWheel: function () {
    if (this.data.currentGroup) {
      wx.navigateTo({
        url: `/pages/tools/wheel?groupId=${this.data.currentGroup.id}`
      })
    } else {
      // 如果没有指定群组，让用户选择
  ...
  navigateToPrice: function () {
    if (this.data.currentGroup) {
      wx.navigateTo({
        url: `/pages/tools/price?groupId=${this.data.currentGroup.id}`
      })
    } else {
      // 如果没有指定群组，让用户选择
  ...
  selectGroupForTool: function (toolType) {
    ...
    wx.showActionSheet({
      ...
      success: (res) => {
        ...
        if (toolType === 'wheel') {
          wx.navigateTo({
            url: `/pages/tools/wheel?groupId=${selectedGroup.id}`
          })
        } else if (toolType === 'price') {
          wx.navigateTo({
            url: `/pages/tools/price?groupId=${selectedGroup.id}`
          })
        }
      }
    })
