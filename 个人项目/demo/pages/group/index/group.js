...
Page({
    ...
    wx.cloud.callFunction({
      ...
      success: (res) => {
        ...
        if (res.result.success) {
          ...
          // 跳转到群组详情页
          wx.navigateTo({
            url: `/pages/group/detail/detail?id=${res.result.data.groupId}`
          })
        } else {
  ...
  performJoinGroup: function (group) {
    ...
    if (groupIndex > -1) {
      ...
      // 跳转到群组详情页
      wx.navigateTo({
        url: `/pages/group/detail/detail?id=${group.id}`
      })
    }
  },
  ...
  navigateToTools: function () {
    ...
    if (currentGroup) {
      wx.navigateTo({
        url: `/pages/tools/index/tools?groupId=${currentGroup._id || currentGroup.id}`
      })
    } else {
      wx.navigateTo({
        url: '/pages/tools/index/tools'
      })
    }
  }
